...
class Administrator extends CI_Controller {
    ...
    function iklansidebar(){
        ...
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_where_ordering('pasangiklan','id_pasangiklan <> 1','id_pasangiklan','DESC');
	...
	//====== module kepala instansi =============
	function kepalainstansi(){
        cek_session_akses('kepalainstansi',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_where_ordering('pasangiklan','id_pasangiklan = 1','id_pasangiklan','DESC');
        }
        $this->template->load('administrator/template','administrator/mod_kepalainstansi/view_kepalainstansi',$data);
    }


    function edit_kepalainstansi(){
        cek_session_akses('kepalainstansi',$this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])){
            $config['upload_path'] = 'asset/foto_pasangiklan/';
            ...
            $where = array('id_pasangiklan' => $this->input->post('id'));
            $this->model_app->update('pasangiklan', $data, $where);
            redirect('administrator/edit_kepalainstansi');
        }else{
            if ($this->session->level=='admin'){
                $proses = $this->model_app->edit('pasangiklan', array('id_pasangiklan' => $id))->row_array();
            }else{
            ...
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_kepalainstansi/view_kepalainstansi_edit',$data);
        }
    }
